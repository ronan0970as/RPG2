(function () {
    ...
    // ════════════════════════════════════════════════════════
    //  ui-enhancements.js  (v2 — bugs de carregamento corrigidos)
    //
    //  CORREÇÕES:
    //  [BUG-UI-1] injectStatusTools() rodava antes de calcularStatus()
    //             ser definido em java.js, fazendo o wrapper falhar
    //             silenciosamente e as barras de status nunca atualizarem.
    //             Agora aguarda com _aguardarCalcStatus() antes de injetar.
    //
    //  [BUG-UI-2] addStatusMeters() observava vida-maxima/mana-maxima
    //             com MutationObserver mas não desconectava ao trocar de
    //             ficha — múltiplos observers acumulavam. Agora guarda e
    //             desconecta observers anteriores.
    //
    //  [BUG-UI-3] injectThemePicker() não reaplicava o tema se o
    //             body.backgroundImage já havia sido definido pelo HTML
    //             inline — agora força reaplicação em onReady.
    //
    //  [BUG-UI-4] initQuickNotes() escrevia no localStorage antes de
    //             getFichaId() estar disponível, salvando na chave errada.
    //             Adiado para depois de _aguardarCalcStatus().
    // ════════════════════════════════════════════════════════

    const THEMES = {
        sombria: {
            label: 'Sombria', image: 'torre_status.png', position: 'center center',
            overlay: 'linear-gradient(to bottom, rgba(4,3,2,0.50), rgba(6,4,2,0.32), rgba(4,3,2,0.58))',
            vars: { '--gold':'#d4a943','--gold-light':'#f5d06e','--gold-dark':'#a07828','--gold-glow':'rgba(212,169,67,0.45)',
                    '--parchment':'#f5e8c0','--ink':'#d4c8a0','--ink-dim':'#9a8e6e','--ink-faint':'#4a4230',
                    '--panel-bg':'rgba(14,11,7,0.78)','--panel-border':'rgba(160,120,40,0.55)','--input-bg':'rgba(8,6,3,0.72)' }
        },
        cristal: {
            label: 'Cristal', image: 'torre_painel.png', position: 'center center',
            overlay: 'linear-gradient(to bottom, rgba(4,2,8,0.48), rgba(6,3,12,0.30), rgba(3,2,6,0.58))',
            vars: { '--gold':'#b06ee8','--gold-light':'#d4a8f5','--gold-dark':'#7c3aed','--gold-glow':'rgba(176,110,232,0.50)',
                    '--parchment':'#ede0ff','--ink':'#c9b8e8','--ink-dim':'#9a82c0','--ink-faint':'#3d2a5a',
                    '--panel-bg':'rgba(10,5,20,0.82)','--panel-border':'rgba(130,80,220,0.50)','--input-bg':'rgba(6,3,14,0.75)' }
        },
        alvorada: {
            label: 'Alvorada', image: 'torre_itens.png', position: 'center center',
            overlay: 'linear-gradient(to bottom, rgba(8,10,5,0.32), rgba(8,9,5,0.20), rgba(5,6,3,0.45))',
            vars: { '--gold':'#caa24a','--gold-light':'#f2d581','--gold-dark':'#8f7730','--gold-glow':'rgba(242,213,129,0.34)',
                    '--parchment':'#fff1c9','--ink':'#dfd09d','--ink-dim':'#a89b6c','--ink-faint':'#564d2d',
                    '--panel-bg':'rgba(14,13,7,0.76)','--panel-border':'rgba(202,162,74,0.48)','--input-bg':'rgba(8,7,3,0.72)' }
        },
        arcana: {
            label: 'Arcana', image: 'torre_talentos.png', position: 'center top',
            overlay: 'linear-gradient(to bottom, rgba(6,3,12,0.46), rgba(8,4,16,0.30), rgba(5,2,10,0.56))',
            vars: { '--gold':'#9f7aea','--gold-light':'#d6bcfa','--gold-dark':'#6b46c1','--gold-glow':'rgba(159,122,234,0.48)',
                    '--parchment':'#f0e9ff','--ink':'#cfc2ef','--ink-dim':'#9f8ec2','--ink-faint':'#44335e',
                    '--panel-bg':'rgba(11,7,22,0.80)','--panel-border':'rgba(130,95,220,0.52)','--input-bg':'rgba(7,4,15,0.76)' }
        }
    };
    ...
    function getActiveFichaId() {
        try { if (typeof window.getFichaId === 'function') return window.getFichaId() || 'global'; } catch(e) {}
        return localStorage.getItem('rpg_ficha_ativa') || 'global';
    }

    function scopedKey(name) { return `${getActiveFichaId()}_${name}`; }
    ...
    // [BUG-UI-3] Sempre força reaplicação do tema no onReady
    function injectThemePicker() {
        ...
        panel.innerHTML = `
            <div class="ui-ambient-title"><span>Ambiente da torre</span></div>
    ...
    function statusValue(key) {
        const map = { vida:'vida-atual', mana:'mana-atual', sanidade:'sanidade-atual' };
    ...
    function setStatusValue(key, value) {
        const ids = { vida:'vida-atual', mana:'mana-atual', sanidade:'sanidade-atual' };
        const el  = document.getElementById(ids[key]);
        if (!el) return;
        const next = Math.max(0, Math.min(statusMax(key), Math.round(value)));
        el.value = String(next);
        el.dispatchEvent(new Event('input',  { bubbles: true }));
        el.dispatchEvent(new Event('change', { bubbles: true }));
    }
    function markSheetChanged() {
        try { if (typeof window.marcarAlterada === 'function') window.marcarAlterada(); } catch(e) {}
    }

    function refreshStatusMeters() {
        const keyMap = { vida:'vida-atual', mana:'mana-atual', sanidade:'sanidade-atual' };
    ...
    // [BUG-UI-2] Guarda observers para desconectar se necessário
    const _statusObservers = [];

    function addStatusMeters() {
        // Desconecta observers anteriores
        _statusObservers.forEach(o => o.disconnect());
        _statusObservers.length = 0;
        ...
        ['vida-maxima','mana-maxima'].forEach(id => {
            ...
            if (!target) return;
            const obs = new MutationObserver(() => refreshStatusMeters());
            obs.observe(target, { childList: true, characterData: true, subtree: true });
            _statusObservers.push(obs);
        });
    ...
    // [BUG-UI-1] Aguarda calcularStatus existir antes de injetar painel
    function _aguardarCalcStatus(tentativas) {
        tentativas = tentativas || 0;
        return new Promise((resolve) => {
            if (typeof window.calcularStatus === 'function') { resolve(); return; }
            if (tentativas > 40) { resolve(); return; } // desiste após 4s mas não bloqueia
            setTimeout(() => _aguardarCalcStatus(tentativas + 1).then(resolve), 100);
        });
    }

    async function injectStatusTools() {
        if (!document.getElementById('vida-atual')) return;
        if (document.getElementById('status-smart-panel')) return;

        // [BUG-UI-1] Aguarda java.js expor calcularStatus
        await _aguardarCalcStatus();
        ...
        panel.innerHTML = `
            <div class="status-smart-grid">
                <div class="status-tool-block">
                    <div class="status-tool-title">Ações rápidas</div>
                ...
                <div class="status-tool-block">
                    ...
                    <div class="dice-controls">
                        <input id="dice-expression" type="text" value="1d20" inputmode="text"
                               autocomplete="off" aria-label="Expressão de dados">
            ...
            <div class="status-tool-block quick-notes">
                <div class="status-tool-title">Notas rápidas</div>
                <textarea id="quick-session-notes" placeholder="Objetivos, pistas, efeitos temporários..."></textarea>
        ...
        panel.querySelector('[data-status-rest]')?.addEventListener('click', () => {
            ...
            setStatusValue('sanidade', Math.min(100, statusValue('sanidade') + 10));
            markSheetChanged();
            refreshStatusMeters();
        ...
        // [BUG-UI-4] Notas rápidas só depois de getFichaId estar disponível
        requestAnimationFrame(() => initQuickNotes(panel));

        // Envolve calcularStatus para atualizar os meters automaticamente
        if (typeof window.calcularStatus === 'function' && !window.calcularStatus._uiEnhanced) {
            ...
            // Atualiza o debounced se existir
            if (typeof window.calcularStatusDebounced === 'function' && !window.calcularStatusDebounced._uiEnhanced) {
                let _t;
                const fn = window.calcularStatus;
                window.calcularStatusDebounced = function(...a) {
                    clearTimeout(_t);
                    _t = setTimeout(() => fn.apply(this, a), 350);
                };
                window.calcularStatusDebounced._uiEnhanced = true;
            }
        }
    }
    ...
    function rollExpression(expression) {
        ...
        if (!clean || !/^[0-9d+\-]+$/.test(clean)) throw new Error('Use algo como 1d20, 2d6+3 ou 1d8-1.');
        ...
        parts.forEach(part => {
            ...
            if (term.includes('d')) {
                ...
                if (!Number.isInteger(count)||!Number.isInteger(sides)||count<1||count>50||sides<2||sides>1000)
                    throw new Error('Limite: até 50 dados, com faces entre 2 e 1000.');
                const rolls = Array.from({ length: count }, () => Math.floor(Math.random() * sides) + 1);
                total += sign * rolls.reduce((a,n) => a+n, 0);
                details.push(`${sign<0?'-':''}${count}d${sides} [${rolls.join(', ')}]`);
            } else {
                const number = parseInt(term, 10);
                if (!Number.isInteger(number)) throw new Error('Expressão inválida.');
                total += sign * number;
    ...
    function getDiceHistory() {
        try { const h = JSON.parse(localStorage.getItem(scopedKey('rpg_dice_history')) || '[]'); return Array.isArray(h) ? h : []; }
        catch(e) { return []; }
    }
    function saveDiceHistory(entry) {
        const h = getDiceHistory();
        h.unshift(entry);
        localStorage.setItem(scopedKey('rpg_dice_history'), JSON.stringify(h.slice(0, 8)));
        renderDiceHistory();
    }
    function renderDiceHistory() {
        const wrap = document.getElementById('dice-history');
        if (!wrap) return;
        wrap.innerHTML = getDiceHistory().map(item => `<span>${item}</span>`).join('');
    }
    function initDiceRoller(panel) {
        ...
        input?.addEventListener('keydown', e => { if (e.key === 'Enter') roll(); });
        panel.querySelectorAll('[data-dice-preset]').forEach(btn => {
            btn.addEventListener('click', () => { input.value = btn.dataset.dicePreset; roll(); });
        });
        renderDiceHistory();
    }

    // [BUG-UI-4] initQuickNotes chamado após rAF para garantir getFichaId disponível
    ...
    // ── Init ──────────────────────────────────────────────
    onReady(() => {
        // [BUG-UI-3] Injeta tema antes de tudo para evitar flash
        injectThemePicker();
        // Aplica tema salvo (ou padrão da página)
        applyTheme(localStorage.getItem('rpg_tower_theme') || defaultThemeForPage(), false);
        // Painel de status é assíncrono (aguarda calcularStatus)
        injectStatusTools();
    });
})();
