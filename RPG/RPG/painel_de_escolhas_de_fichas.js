...
async function criarFichaSupabase({ nome, classe, img }) {
    ...
    const { data, error } = await window._supaClient
        .from('fichas')
        .insert([{ user_id: userId, nome, classe, img }])
        .select()
        .single();

    if (error) { console.error('Erro ao criar ficha:', error.message); return; }

    fichas.push({ id: data.id, nome: data.nome, classe: data.classe, img: data.img || '' });
    renderizar();
}
...
async function criarFicha() {
    ...
    fecharModal();
    await criarFichaSupabase({ nome, classe, img: imgUrl });
}
