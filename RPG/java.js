        ...
        listaConteiner.innerHTML += `
            <div class="item-card" id="item-card-${item.id}">
                ...
                <div style="display:flex;flex-direction:column;gap:8px;align-self:center;">
                    <button class="btn-editar-item" onclick="editarItemInventario(${item.id})">✏️ Editar</button>
...
// Abre o formulário de edição inline dentro do card
async function editarItemInventario(id) {
    ...
    card.innerHTML = `
        <div style="display:flex;flex-direction:column;gap:10px;width:100%;">
            <input type="text" id="edit-nome-${id}" value="${item.nome.replace(/"/g,'&quot;')}"
                   placeholder="Nome do item" style="font-size:15px;">
            <textarea id="edit-desc-${id}" rows="4"
                      style="font-size:15px;resize:vertical;">${item.descricao}</textarea>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                <input type="text" id="edit-img-${id}" value="${item.imagem.startsWith('data:') ? '' : item.imagem}"
                       placeholder="URL da imagem (ou carregue abaixo)" style="flex:1;min-width:160px;font-size:14px;">
                <input type="file" id="edit-file-${id}" accept="image/*"
                       onchange="editCarregarImagem(${id})" style="font-size:13px;background:transparent;border:none;color:#888;cursor:pointer;">
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
                <label style="font-size:12px;color:#c8aa6e;width:auto;">🔍 Zoom</label>
                <input type="range" id="edit-zoom-${id}" min="1" max="3" step="0.1" value="${item.zoom||1}"
                       style="flex:1;accent-color:#c8aa6e;height:32px;" oninput="editAtualizarPreviewImg(${id})">
                <label style="font-size:12px;color:#c8aa6e;width:auto;">↔️ H</label>
                <input type="range" id="edit-posx-${id}" min="0" max="100" value="${item.posX||50}"
                       style="flex:1;accent-color:#c8aa6e;height:32px;" oninput="editAtualizarPreviewImg(${id})">
                <label style="font-size:12px;color:#c8aa6e;width:auto;">↕️ V</label>
                <input type="range" id="edit-posy-${id}" min="0" max="100" value="${item.posY||50}"
                       style="flex:1;accent-color:#c8aa6e;height:32px;" oninput="editAtualizarPreviewImg(${id})">
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="font-size:12px;color:#888;">Prévia:</span>
                <div style="width:80px;height:80px;border-radius:4px;border:1px solid #333;overflow:hidden;background:#111;flex-shrink:0;">
                    <img id="edit-prev-${id}" src="${item.imagem}"
                         style="width:100%;height:100%;object-fit:cover;transform:scale(${item.zoom||1});object-position:${item.posX||50}% ${item.posY||50}%;">
                </div>
            </div>
            <div style="display:flex;gap:8px;">
                <button class="save-btn" style="margin-top:0;font-size:14px;padding:10px;"
                        onclick="salvarEdicaoItem(${id})">💾 Salvar</button>
                <button class="btn-remover-item" style="flex:1;font-size:14px;"
                        onclick="renderizarInventario()">✕ Cancelar</button>
            </div>
        </div>`;
}

// Carrega imagem local no formulário de edição
function editCarregarImagem(id) {
    ...
    reader.onload = function(e) {
        ...
        img.onload = function() {
            ...
            document.getElementById(`edit-img-${id}`).value = base64;
            const prev = document.getElementById(`edit-prev-${id}`);
...
// Atualiza prévia da imagem ao mexer nos sliders
function editAtualizarPreviewImg(id) {
    const zoom = document.getElementById(`edit-zoom-${id}`)?.value || 1;
    const posX = document.getElementById(`edit-posx-${id}`)?.value || 50;
    const posY = document.getElementById(`edit-posy-${id}`)?.value || 50;
    const imgUrl = document.getElementById(`edit-img-${id}`)?.value;
    const prev = document.getElementById(`edit-prev-${id}`);
    if (!prev) return;
    if (imgUrl) prev.src = imgUrl;
    prev.style.transform = `scale(${zoom})`;
    prev.style.objectPosition = `${posX}% ${posY}%`;
}
...
async function salvarEdicaoItem(id) {
    ...
    const imgEl  = document.getElementById(`edit-img-${id}`);
    const zoom   = document.getElementById(`edit-zoom-${id}`)?.value || 1;
    const posX   = document.getElementById(`edit-posx-${id}`)?.value || 50;
    const posY   = document.getElementById(`edit-posy-${id}`)?.value || 50;

    if (!nomeEl || !descEl) return;
    const nome = nomeEl.value.trim();
    const desc = descEl.value.trim();
    if (!nome || !desc) { alert('Preencha o nome e a descrição!'); return; }

    let inventario = await carregarDaNuvem('inventario') || [];
    const idx = inventario.findIndex(i => i.id === id);
    if (idx === -1) return;

    const imgAtual = inventario[idx].imagem;
    const novaImg  = imgEl?.value.trim() || imgAtual;

    inventario[idx] = { ...inventario[idx], nome, descricao: desc, imagem: novaImg, zoom, posX, posY };
    localStorage.setItem(k('rpg_inventario'), JSON.stringify(inventario));
    await salvarNuvem('inventario', inventario);
    renderizarInventario();
}
